...
def handle_headlines(client_socket):
    ...
    if choice == "1":
        ...
        category = input(f"Enter category {VALID_CATEGORIES}: ").lower()
        if category not in VALID_CATEGORIES:
            print(f"Invalid category. Choose from {VALID_CATEGORIES}.")
            return
        request = {"action": "headlines", "category": category}
    elif choice == "3":
        country = input(f"Enter country code {VALID_COUNTRIES}: ").lower()
        if country not in VALID_COUNTRIES:
            print(f"Invalid country code. Choose from {VALID_COUNTRIES}.")
            return
...
def handle_sources(client_socket):
    ...
    if choice == "1":
        ...
        language = input(f"Enter language code {VALID_LANGUAGES}: ").lower()
        if language not in VALID_LANGUAGES:
            print(f"Invalid language code. Choose from {VALID_LANGUAGES}.")
            return
